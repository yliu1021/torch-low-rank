...
def main(args):
    ...
    (train_x, train_y), (val_x, val_y) = data.load_data("cifar10")
    ...
    train_metrics = model.fit(
        train_x, train_y, batch_size=args.batch_size, epochs=args.pre_prune_epochs
    )
    ...
    for i, layer in enumerate(model.layers):
        if issubclass(type(layer), LowRankLayer):
            ...
            layer.set_rank(
                new_rank=int(layer.max_rank * args.pruning_ratio),
                reuse_existing_kernels=True,
            )
            print(
                "Layer "
                + str(i)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--method",
        type=int,
        help="Method to score ranks by: " + str(list(SCORING_METHOD)),
    )
    parser.add_argument(
        "--pre_prune_epochs",
        type=int,
        help="Number of epochs to train for before pruning",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pruning_ratio",
        type=float,
        help="Ratio of singular vectors pruned from each layer",
    )
